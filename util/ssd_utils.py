...
class BBoxUtility(object):
    ...
    def assign_boxes(self, boxes):
        ...
        # loc
        assignment[:, -12:-8][best_iou_mask] = encoded_boxes[best_iou_idx, np.arange(assign_num), :4]
        # background
        assignment[:, 0][best_iou_mask] = 0

        # label
        assignment[:, 1:-12][best_iou_mask] = boxes[best_iou_idx, 4:]
    ...
    def decode_boxes(self, mbox_loc, mbox_priorbox, variances):
        ...
        decode_bbox = np.concatenate((decode_bbox_xmin[:, None], decode_bbox_ymin[:, None], decode_bbox_xmax[:, None],
    ...
    def decode_boxes2(self, mbox_loc, mbox_priorbox):
        """
        Convert bboxes from local predictions to shifted priors.
        # Arguments
            mbox_loc: Numpy array of predicted locations.
    ...
    def detection_out2(self, predictions, background_label_id=0, keep_top_k=200,
        ...
        mbox_loc = predictions[:, :, -4:]
        # variances = predictions[:, :, -4:]
        mbox_priorbox = self.priors
        mbox_conf = predictions[:, :, :-4]
        results = []
        for i in range(len(mbox_loc)):
            results.append([])
            decode_bbox = self.decode_boxes2(mbox_loc[i],
                                             mbox_priorbox)
            # mbox_priorbox[i])
            for c in range(self.num_classes):
                if c == background_label_id:
