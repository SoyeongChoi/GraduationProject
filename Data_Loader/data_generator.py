...
class DataGenerator:
    ...
    # Data
    def __init__(self, labels_output_format=('class_id', 'xmin', 'ymin', 'xmax', 'ymax'))

        '''
        :param labels_output_format (list, optional): A list of five strings representing the desired order of the five
        ...
        self.data_path = os.path.join('../', 'data', 'Manually_Annotated')

        self.expression_list = ['Neutral', 'Happy', 'Sad', 'Surprise', 'Fear', 'Disgust', 'Anger', 'Contempt', 'None', 'Uncertain',
                       ' Non-face']
        self.expression_size = [75374, 134915, 25959, 14590, 6878, 4303, 25382, 4250, 33588, 12145, 82915]
        ## netural, happy, sad, surprise, anger
        ## 0, 1, 2, 3, 6 [label min size = 14590 (Surprise)/ label max size = 134915(Happy)]

    def init_selection(self, selection = None):
        self.selection
