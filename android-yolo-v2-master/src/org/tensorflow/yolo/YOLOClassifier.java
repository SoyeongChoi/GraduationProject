...
public class YOLOClassifier {
    ...
    private final static double anchors[] = {78.21261596679688,119.09812927246094,  103.61547088623047,156.59695434570312,  112.44337463378906, 170.9337158203125,  44.764163970947266,65.76957702636719,  75.4170150756836,112.6888656616211};
    private final static int SIZE = 13; //13
    private final static int MAX_RECOGNIZED_CLASSES = 1;
    private final static float THRESHOLD = 0.5f;
    private final static int MAX_RESULTS = 15; // 15
    ...
    public List<Recognition> classifyImage(final float[] tensorFlowOutput, final Vector<String> labels) {
        ...
        //Log.e("myTag", Integer.toString(tensorFlowOutput.length));

        int numClass = (int) (tensorFlowOutput.length / (Math.pow(SIZE,2) * NUMBER_OF_BOUNDING_BOX) - 5);
        // Log.e("myTag", Integer.toString(numClass));
        BoundingBox[][][] boundingBoxPerCell = new BoundingBox[SIZE][SIZE][NUMBER_OF_BOUNDING_BOX];
        // Log.e("myTag", Integer.toString(SIZE));
        // Log.e("myTag", Integer.toString(NUMBER_OF_BOUNDING_BOX));
