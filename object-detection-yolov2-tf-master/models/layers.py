...
def conv2d(x, W, stride, padding='SAME'):
    ...
    :return: tf.Tensor.
    """
    return tf.nn.conv2d(x, W, stride, padding=padding)
...
def conv_layer(x, side_l, stride, out_depth, padding='SAME', use_bias=True, **kwargs):
    ...
    if use_bias:
        ...
        return conv2d(x, filters, stride = [1, stride, stride, 1], padding=padding) + biases
    else:
        return conv2d(x, filters, stride = [1, stride, stride, 1], padding=padding)

def depthwise_conv2d(x, in_depth, out_depth, strides, padding='SAME', use_bias=True, name=None, **kwargs):
    """
    Add a new convolutional layer.
    :param x: tf.Tensor, shape: (N, H, W, C).
    :param side_l: int, the side length of the filters for each dimension.
    ...
    weights_stddev = kwargs.pop('weights_stddev', 0.01)
    in_depth = int(x.get_shape()[-1])
    filters = weight_variable([3, 3, in_depth, 1], stddev=weights_stddev)
    if use_bias:
        biases_value = kwargs.pop('biases_value', 0.1)
        biases = bias_variable([out_depth], value=biases_value)
        return tf.nn.depthwise_conv2d(x, filters, strides, padding=padding) + biases
    else:
        return tf.nn.depthwise_conv2d(x, filters, strides, padding=padding)


def batchNormalization(x, is_train, is_name):
    """
    ...
    return tf.nn.batch_normalization(x, mean, variance, offset=None, scale=None, variance_epsilon=1e-8, name=is_name)
...
def moments(
  ...
  with ops.name_scope(name, "moments", [x, axes]):
    ...
    variance = math_ops.reduce_mean(
        pow(y - array_ops.stop_gradient(mean),2),
        axes,
        keepdims=True,
        name="variance")
    if not keep_dims:
