...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        classification_loss = tf.to_float(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-4])) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.to_float(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-4:])) # Output shape: (batch_size, n_boxes)
