...
def build_keras_model(input_shape, num_classes):
    ...
    f1_conf = tf.keras.layers.Conv2D((3 * (class_num)), kernel_size=3, activation='relu', padding="same",
                                     use_bias=False)(x)
    f1_conf = tf.keras.layers.Reshape((19 * 19 * 3, (class_num)), input_shape=(19, 19, 3 * (class_num)),
    ...
    f2_conf = tf.keras.layers.Conv2D((6 * (class_num)), kernel_size=3, activation='relu', padding="same",
                                     use_bias=False)(x)
    f2_conf = tf.keras.layers.Reshape((10 * 10 * 6, (class_num)), input_shape=(10, 10, 6 * (class_num)),
    ...
    f3_conf = tf.keras.layers.Conv2D((6 * (class_num)), kernel_size=3, activation='relu', padding="same",
                                     use_bias=False)(x)
    f3_conf = tf.keras.layers.Reshape((5 * 5 * 6, (class_num)), input_shape=(5, 5, 6 * (class_num)), name='f3_conf')(
        f3_conf)
    ...
    f4_conf = tf.keras.layers.Conv2D((6 * (class_num)), kernel_size=3, activation='relu', padding="same",
                                     use_bias=False)(x)
    f4_conf = tf.keras.layers.Reshape((3 * 3 * 6, (class_num)), input_shape=(3, 3, 6 * (class_num)), name='f4_conf')(
        f4_conf)
    ...
    f5_conf = tf.keras.layers.Conv2D((6 * (class_num)), kernel_size=3, activation='relu', padding='same',
                                     use_bias=False)(x)
    f5_conf = tf.keras.layers.Reshape((2 * 2 * 6, (class_num)), input_shape=(1, 1, 6 * (class_num)), name='f5_conf')(
        f5_conf)
    ...
    f6_conf = tf.keras.layers.Conv2D((6 * (class_num)), kernel_size=3, activation='relu', padding='same',
                                     use_bias=False)(x)
    f6_conf = tf.keras.layers.Reshape((1 * 1 * 6, (class_num)), input_shape=(1, 1, 6 * (class_num)), name='f6_conf')(
        f6_conf)

    z_loc = tf.keras.layers.concatenate([f1_loc, f2_loc, f3_loc, f4_loc, f5_loc, f6_loc], axis=1, name='total_loc')

    z_conf = tf.keras.layers.concatenate([f1_conf, f2_conf, f3_conf, f4_conf, f5_conf, f6_conf], axis=1,
                                         name='total_conf')
